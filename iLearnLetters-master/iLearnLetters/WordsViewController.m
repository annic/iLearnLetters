...
- (void)viewDidLoad
{
    ...
    [refreshButton setBackgroundColor: backgroundColor];
    [soundButton setBackgroundColor:backgroundColor];
...
-(NSString *)randomlyPickWord{
    ...
    for (int i = 0; i < [[self.arrayOfWords objectAtIndex:r] length]; i++) {
        [self createButtonFront:i:[[self.arrayOfWords objectAtIndex:r] length] :[NSString stringWithFormat:@"%@", [[self.arrayOfWords objectAtIndex:r] substringWithRange:NSMakeRange(i, 1)]]];
...
-(void)createButtonFront:(int)whichLetter :(int) arrLen :(NSString *)letter{
    
    float midLen = arrLen / 2;
    
    CGPoint pointToAddButton = CGPointMake(500 + ((float)whichLetter - midLen) * 120, 200);
    ...
    buttonToAdd.titleLabel.font = [UIFont boldSystemFontOfSize:82];
